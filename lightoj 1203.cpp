...
void solve()
{
    ...
    int m=getConvexHull(p,n,con_hull);
    if(m<3)
    {
        printf("Case %d: 0\n",++ii );
        return;
    ...
    for(int i=0;i<m;i++)
    {
        ...
        if(prv<0)prv+=m;
        int nxt=(i+1)%m;
        double a=getDistance(con_hull[prv],con_hull[nxt]);
        double b=getDistance(con_hull[i],con_hull[nxt]);
        double c=getDistance(con_hull[prv],con_hull[i]);
